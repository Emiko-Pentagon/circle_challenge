...
// import { sliceHash } from '../utils/web3';
...
//         if (existingUser) {
//             if (existingUser.address.toLocaleLowerCase() !== req.query.state.toLocaleLowerCase())
//                 return done(`Invalid account. This user is connected to: ${sliceHash(existingUser.address)}.`, null);

//             existingUser.google = profile.id;
//             existingUser.firstName = profile.name.givenName;
//             existingUser.lastName = profile.name.familyName ? profile.name.familyName : 'N/A';
...
//             return done('User already registered with this email', null);
//         } else {
...
//             if (!isAddress)
//                 return done('Invalid address provided', null);
...
//             user.lastName = profile.name.familyName ? profile.name.familyName : 'N/A';
